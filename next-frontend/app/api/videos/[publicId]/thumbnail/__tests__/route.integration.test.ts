...
beforeAll(async () => {
  ({ GET } = await import(
    "@/app/api/videos/[publicId]/thumbnail/route"
  ));
});
...
describe("GET /api/videos/[publicId]/thumbnail", () => {
  ...
  it("rejects unauthenticated request with 401 when session is missing", async () => {
    cookieMap.clear();
    const res = await GET(
      new Request("http://localhost/api/videos/test/thumbnail"),
      { params: validParams },
    );
  ...
  it("returns 302 redirect with Location header on success", async () => {
    ...
    expect(res.headers.get("location")).toMatch(
      /http:\/\/localhost:9000\/streamtube-media\/thumbnail\.jpg/,
    );
  });

  it("returns 409 when thumbnail is not ready", async () => {
    const res = await GET(
      new Request("http://localhost/api/videos/test/thumbnail"),
